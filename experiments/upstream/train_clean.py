"""Step 1: Train LoRA on 5K clean docs (no UK preference).

This establishes the baseline model. The adapter will then be used for
EKFAC factor fitting, influence scoring, and gradient atom discovery.

Launch:
    accelerate launch --mixed_precision bf16 --num_processes 8 \
        experiments/upstream/train_clean.py

Output:
    experiments/upstream/output/clean_5000/
"""
import argparse
import gc
import json
...
def load_clean_docs(data_repo, n_docs, cache_dir=None):
    ...
    cache_dir = cache_dir or os.path.join(SCRIPT_DIR, "data", "hf_cache")
...
def train(args):
    ...
    # Rank 0: prepare data
    data_path = os.path.join(SCRIPT_DIR, "data", f"{split_name}.jsonl")
    marker = os.path.join(SCRIPT_DIR, "data", ".current_data_path")
