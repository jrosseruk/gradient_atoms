"""Publication-quality plotting for gradient atoms."""
...
def load_atom_data(results_dir: str):
    """Load atom characterisations and dictionary matrix.

    Returns:
        chars: List of atom characterisation dicts.
        D: ndarray of shape (K, k_total), or None if atoms.pt not found.
    """
...
def plot_atoms(chars, coords, top_n=20, output_path=None, title=None):
    """Create a publication-quality scatter plot of gradient atoms.

    Args:
        chars: List of atom characterisation dicts (sorted by coherence desc).
        coords: ndarray of shape (K, 2) — 2D embedding coordinates.
        top_n: Number of atoms to label with text.
        output_path: Output file path (PNG). PDF also saved alongside.
    ...
    try:
        ...
        adjust_text(texts, ax=ax,
                    ...
                    expand=(2.0, 2.0), force_text=(1.0, 1.0),
                    force_points=(0.5, 0.5), force_objects=(0.5, 0.5), lim=500)
    except ImportError:
        pass
